...
print('[Checkpoint 01] Created socket at ' + str(host) + 'on port ' + str(port))
s.bind((host, port))
s.listen(backlog)
...
wolf_client = wolframalpha.Client('GY42EE-55Q8L88P47')
	
while 1:
	# Receive payload from the client
	client,address = s.accept()
	print('[Checkpoint 03] Accepted client connection from ' + str(host) + 'on port ' + str(port))
	...
	if (Server_Question_checkSum == Server_Question_recheckSum):
		...
		print('[Checkpoint 05] Decrypt: ' + str(key) + ' | Plain text: ' + str(decryptedMes))
		...
		print('[Checkpoint 06] Speaking Question: ' + str(decryptedMes))
		os.system("omxplayer Question.mp3")
		
		# Get answer from WolframeAlpha API
		print('[Checkpoint 07] Sending question to Wolframalpha: ' + str(decryptedMes))
		res = wolf_client.query('Question: ' + decryptedMes)
		ans = next(res.results).text
		print('[Checkpoint 08] Received answer from Wolframalpha: ' + str(ans))
		ans = str.encode(ans)
			
		# Encrypt and hash answer 
		f_answer_encrypt = Fernet(key)
		Server_encrypted_ans = f_answer_encrypt.encrypt(ans)
		print('[Checkpoint 09] Encrypt: Key: ' + str(key) + ' | Ciphertext: ' + str(Server_encrypted_ans))
		m_ans = hashlib.md5()
		m_ans.update(Server_encrypted_ans)
		Server_Ans_checkSum = m_ans.digest()
		print('[Checkpoint 10] Generated MD5 Checksum: ' + str(Server_Ans_checkSum))
		...
		print('[Checkpoint 11] Sending answer: ' + str(Serialized_ans_payload))
